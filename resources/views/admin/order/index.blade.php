          ...
          $currency = $order->currency;
           ...
           <tr>
            @foreach($column_array as $key => $value)
              ...
              @elseif ($key=='proofreader_id')
                <td>{!! $order->$key>0?$currency_with_icon_array[$order->currency]:'' !!} {!! get_field_value('av_earning_tbl','amount','order_id',$order->id) !!}</td>
              @elseif ($key=='id')
                <td>{!! get_orderID($order->$key) !!}</td>
              @elseif ($key=='user_id')              
              ...
              @elseif($key=='status')
                <td>{!! $order_status_array[$order->$key] !!}</td>
            ...
            <td>
              <a href="{{ url('/admin/order/view/'.$order->id) }}" title="View"><i class="fa fa-fw fa-eye"></i></a>
              <a href="{{ url('/admin/order/delete/'.$order->id) }}" onclick="return confirm('Are you sure?');" title="Delete"><i class="fa fa-fw fa-close"></i></a>
            </td>

          </tr>
          @endforeach
          @if(Request()->sdate || Request()->edate || Request()->status)
          <tr>
            <td colspan="3">Total Order Amount </td>
            <td>{!! $currency_with_icon_array[$currency] !!} {{$total_order_amount}}</td>
            <td colspan="<?php echo count($column_array)-3;?>"> </td>
          </tr>
          <tr>
            <td colspan="3">Total Proofreader Earn Amount </td>
            <td>{!! $currency_with_icon_array[$currency] !!} {{$proofreader_amount}}</td>
            <td colspan="<?php echo count($column_array)-3;?>"> </td>
          </tr>
          <tr>
            <td colspan="3">Total Cancelled Amount </td>
            <td>{!! $currency_with_icon_array[$currency] !!} {{$total_cancel_amount}}</td>
            <td colspan="<?php echo count($column_array)-3;?>"> </td>
          </tr>
          @endif
