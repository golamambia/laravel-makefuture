...
<div class="apply_form_area">
    <div class="container" style="max-width: 790px;">
        <div class="apply_form">
            <h2 class="heading">{{ __('Sign up to get your work checked') }}</h2>
            <div class="form">
            @if($errors->any())   
            ...
            <form method="POST" action="{{ route('register') }}" class="customvalidation">
                ...
                <div class="form-group">
                    <input type="text" class="form-control" name="referral_code" value="{{ old('referral_code') }}" placeholder="Have a Referral code?">
                    @error('referral_code_validate')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
